...
switch lower(job_struct.batch_config.software)
    ...
    case {'matlab','octave'}
        ...
        %% OBSOLETE... EF_GEN_M IS EXECUTED BY POP_RUNHTB... RUN EF_GEN_M...
        %job_struct=ef_gen_m(job_struct);
        
        %% COMPLETE SUBSTRINIT... UPDATE THIS TO BASH SCRIPT!!!
%        subStrInit=sprintf('%s\n\n%s%s %s;\n%s %s;\n%s %s %s %s;\n\n', ...
%            '#!/bin/bash', ...
%            subStrInit, ...
%            'cd',job_struct.remote_work, ...
%            'mkdir',fullfile(job_struct.context_config.log,job_struct.m_path), ...
%            'unzip',[fullfile(job_struct.context_config.log,job_struct.m_path),'.zip'], ...
%            '-d',fullfile(job_struct.context_config.log,job_struct.m_path) ...
%            );
